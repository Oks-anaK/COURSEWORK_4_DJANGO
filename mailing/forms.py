...
class MailingForm(ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if user:
            ...
            if is_manager:
                ...
                self.fields['recipients'].queryset = Recipient.objects.all().order_by('username', 'email')
            else:
                # Обычные пользователи видят только своих получателей
                self.fields['recipients'].queryset = Recipient.objects.filter(owner=user).order_by('username', 'email')
        ...
        if user:
            ...
            if is_manager:
                ...
                self.fields['message'].queryset = Message.objects.all().order_by('subject')
            else:
                # Обычные пользователи видят только свои сообщения
                self.fields['message'].queryset = Message.objects.filter(owner=user).order_by('subject')
    ...
    new_message_subject = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Или создайте новое сообщение: введите тему'}),
    ...
    new_message_body = forms.CharField(
        ...
        required=False,
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 4, 'placeholder': 'Введите содержимое письма'}),
    )
    ...
    new_recipient_username = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Или создайте нового получателя: введите ФИО'}),
    ...
    new_recipient_comment = forms.CharField(
        ...
        required=False,
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 2, 'placeholder': 'Введите комментарий'}),
    )
    
    class Meta:
        model = Mailing
        fields = "__all__"
        exclude = ['owner', 'status']  # Владелец и статус устанавливаются автоматически в view
