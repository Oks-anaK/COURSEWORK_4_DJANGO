...
class Message(models.Model):
    subject = CharField(
        max_length=250, verbose_name="Тема письма", help_text="Напишите тему письма."
    )
    body = TextField(
        verbose_name="Содержание письма", help_text="Добавьте содержимое письма."
    )
    owner = models.ForeignKey(
        User,
...
class Mailing(models.Model):
    ...
    status = CharField(
        max_length=50, verbose_name="Статус", blank=True, null=True, default="Создана"
    )
    message = ForeignKey(
        Message,
        on_delete=models.CASCADE,
